...
Sub Stage_DrawActor(actor)
	...
	If Sprite_Animation_NumFrames[sprite, anim] > 1 Then
		t = Timer
		If t - Actor_CurrentFrameTime[actor] > Sprite_Animation_Delay[sprite] Then
	...
	'dx = 0
	If Stage_Layer_ColDet[ Actor_Layer[actor] ] Then
		...
		World_Actor_Collision(actor, Actor_CurrentFrame[actor], Actor_X[actor], Actor_Y[actor], new_frame, Actor_NewX[actor], Actor_NewY[actor])
	...
	x = Sprite_Frame_Bound_X[sprite, frame] + (x-Stage_offset_x)
	y = Sprite_Frame_Bound_Y[sprite, frame] + (y-Stage_offset_y)
	w = Sprite_Frame_Bound_Param[sprite, frame, 0] - Sprite_Frame_Bound_X[sprite, frame]
	h = Sprite_Frame_Bound_Param[sprite, frame, 1] - Sprite_Frame_Bound_Y[sprite, frame]
	Rect(x, y, w, h)
End Sub
...
Function Game_WindowOpen(t$, w, h, flag)
	WindowOpen(0, t$, WINDOWPOS_CENTERED, WINDOWPOS_CENTERED, w, h, flag)
